---
layout: post
title: "Wait Times from Lumpectomy to Adjuvant Radiotherapy in Canada (Alberta & BC)"
date: 2025-11-29
permalink: /litreview/2025/11/29/wait-times-lumpectomy-rt/
categories: litreview oncology healthsystems
thumbnail: /assets/litreview-wait-times.jpg
summary: "Deep dive into Canadian and provincial data on time from lumpectomy to adjuvant radiotherapy, with a focus on Alberta and British Columbia."
series: deep-dive
image: /assets/litreview-wait-times.jpg
---
# Wait Times from Lumpectomy to Adjuvant Radiotherapy in Canada  
---

# Introduction

Timeliness in initiating adjuvant radiotherapy (RT) following breast-conserving surgery (BCS) is a critical determinant of oncologic quality and health-system performance. For early-stage breast cancer patients who do not receive chemotherapy, clinical guidelines universally recommend initiation of RT within **6–12 weeks** after lumpectomy. Delays beyond this window are associated with progressively higher risks of local recurrence and, in extreme cases, inferior survival outcomes. 

Despite substantial investments in radiotherapy infrastructure nationwide, my anecdotal observations suggest that some patients are beginning adjuvant RT **16–17 weeks** after surgery, substantially outside guideline boundaries. Conversely, **British Columbia**, which experienced a severe capacity crisis in 2022–2023, has since undergone a major system recovery.

This analysis synthesizes guideline evidence, pathway timing data, and provincial operational performance to evaluate whether prolonged lumpectomy-to-RT intervals represent isolated outliers or a broader structural issue within Canadian cancer systems.

---

# 1. Clinical Guidelines and the Evidence Base for Timely RT

## 1.1 Guideline Expectations

Across Canadian and international sources, there is strong convergence around an outer limit of **12 weeks** for RT initiation after BCS in the absence of chemotherapy:

- **Canadian national guidelines**: “As soon as possible and not later than **12 weeks**” post-lumpectomy.  
- **NCCN**: Initiate RT **6–8 weeks** after surgery when no systemic therapy intervenes.  
- **European (ESTRO / Dutch)**: Ideal **4–6 weeks**; maximum **12 weeks**.  
- **Operational benchmark (CPAC/CIHI)**: 90% of patients treated within **28 days** of readiness—an operational proxy but *not* a clinically defined interval.

### Clinical Rationale  
- Local recurrence risk increases with each month of delay beyond approximately eight weeks.  
- Delays **>12 weeks** consistently worsen local control across meta-analyses and cohort studies.  
- Very prolonged delays (**>20 weeks**) have demonstrated negative impacts on recurrence-free and overall survival in Canadian population data.

Thus, the evidence supports a biologically and clinically meaningful window for initiating adjuvant RT after BCS.

---

# 2. Conceptualizing the Treatment Pathway  
### Why System Metrics and Clinical Metrics Diverge

The full patient pathway includes multiple sequential intervals:

1. **Surgery → Referral**  
2. **Referral → Radiation Oncology Consult**  
3. **Consult → CT Simulation**  
4. **Simulation → RT Ready Date**  
5. **Ready → First Fraction**

Provincial dashboards, CIHI indicators, and most public wait-time frameworks measure **only the final segment** (Ready→Start). These operational metrics reflect treatment capacity but do **not** capture the clinically relevant delay from **surgery to RT start**.

### Resulting Measurement Blind Spot
- A province may appear to meet all wait-time benchmarks, even while patients experience **substantial delays upstream** (especially referral→consult).
- This blind spot is currently consequential in Alberta.

---

# 3. Alberta: Consultation Bottleneck and Its Systemic Consequences

## 3.1 Historical Performance (Pre-2020)

Prior to the COVID-19 era, Alberta consistently met or exceeded pan-Canadian RT benchmarks:

- **98%** of patients started RT within **28 days** of readiness (2020).  
- **64%** started within **2 weeks**.  
- Implied surgery→RT intervals (for direct-to-RT patients): **6–10 weeks**.

These metrics were sustained by substantial expansion of regional RT centres between 2010–2020.

## 3.2 Current State (2023–2025): A Front-End Access Crisis

Despite continued strong performance in the Ready→Start segment, Alberta now faces **prolonged access delays** to radiation oncology consultation:

- **Referral → consult:** up to **12–13 weeks**, versus a 4-week benchmark.  
- **Once ready:** 90% still treated within ≤28 days.

### Mechanistic Explanation
The **consult bottleneck**, not linac capacity, is the dominant driver of extended surgery→RT intervals. When a patient waits 12–13 weeks for consultation, even a high-functioning treatment service cannot prevent total waits from exceeding guideline limits.

### Resulting Clinical Interval  
Surgery (~week 0) → referral (variable) → consult (week 12–13) → planning (1–2 weeks) → RT start (week 14–17).

**Thus, 16–17 week starts are systemically predictable given current access constraints.**

## 3.3 Structural Drivers
- Rising cancer incidence (+40% since 2013).  
- Only 20% growth in oncology workforce over the same period.  
- Clinic room constraints, particularly at the Cross Cancer Institute (Edmonton).  
- Pandemic-driven surgical delays creating fluctuating referral surges.

Alberta therefore demonstrates a **front-end access failure** embedded within an otherwise functional treatment-capacity system.

---

# 4. British Columbia: Capacity Crisis, Cross-Border Treatment, and Recovery

## 4.1 Crisis Period (2022–2023)

BC Cancer experienced system-wide deterioration due to therapist shortages, oncologist vacancies, and rising volumes:

- Ready→Start performance worsened to **~65–70%** within 28 days.  
- Many breast patients initiated RT **>12–16 weeks** post-surgery.  
- Over **1,100 patients** were treated in Washington State to mitigate delays.

## 4.2 Recovery (2024–2025)

Following aggressive investment:

- **76%** treated within 4 weeks (mid-2024).  
- **93%** treated within 4 weeks (Feb 2025)—exceeding national benchmarks.  
- Cross-border outsourcing ended.

### Key Contrast With Alberta
- **BC’s deficit was treatment capacity** (linacs & therapists).  
- **Alberta’s deficit is pre-treatment access** (physician consult availability).

BC demonstrates that large-scale system remediation can reverse severe wait-time deterioration within ~18–24 months.

---

# 5. Comparing Alberta and BC Through a Health-Systems Lens

| Dimension | Alberta | British Columbia |
|----------|---------|------------------|
| **Primary bottleneck** | Consult access | Treatment capacity |
| **Ready→Start performance** | Strong (≤28 days) | Recovered to strong (93% ≤28 days) |
| **Surgery→RT performance** | Suboptimal (14–17 weeks in many cases) | Improved; now closer to guideline windows |
| **System trajectory** | Worsening consult delays | Rapid recovery after crisis |
| **Underlying driver** | Workforce & clinic constraints | Insufficient RT throughput |

Both systems highlight the importance of distinguishing **operational metrics** from **clinically meaningful intervals**.

---

# 6. Clinical Interpretation: Are 16–17 Week Delays Acceptable?

Based on the guideline synthesis and evidence review:

### 6.1 Within-Historical Norms?  
No. Alberta’s historical median was ~8–10 weeks.

### 6.2 Within Current System Performance?  
Yes—these waits are systemically generated by the consult backlog.

### 6.3 Clinically Acceptable?  
No. These delays exceed the **12-week** upper limit endorsed by all major guidelines and carry increased risk of local recurrence.

### 6.4 Do They Warrant QI Investigation?  
Yes. Any centre with routine post-lumpectomy RT initiation beyond 12 weeks should conduct pathway-level QI analysis to identify bottlenecks.

---

# 7. Obtaining Accurate Surgery→RT Data for Local QI

Clinicians or trainees can access true interval data via:

- **Alberta:** Cancer Care Alberta Analytics / Alberta Cancer Registry  
- **BC:** BC Cancer Surveillance & Outcomes Division  

Recommended minimum dataset:

- Date of lumpectomy  
- Date of referral  
- Date of RO consultation  
- CT simulation date  
- RT start date  

Key calculated intervals:

- Surgery → consult  
- Consult → RT start  
- **Surgery → RT start (primary clinical metric)**

Interpretation thresholds:

- **6–12 weeks:** guideline-concordant  
- **12–16 weeks:** delayed  
- **>16 weeks:** operational/system failure  
- **>20 weeks:** associated with poorer survival in selected cohorts  

Because provincial dashboards lack surgery→RT metrics, such local audits often reveal hidden bottlenecks unnoticed by system leaders.

---

# 8. Implications for Oncology Practice and Health-System Planning

1. **Guideline adherence is no longer guaranteed by meeting RT operational benchmarks.**  
   Strong Ready→Start performance can coexist with clinically unacceptable surgery→RT intervals.

2. **Consult access is a critical rate-limiting step** in modern cancer pathways.  
   Workforce planning must consider both physician FTEs and clinic infrastructure.

3. **British Columbia shows that aggressive investment can restore performance** even after crisis-level deterioration.

4. **Local QI initiatives should incorporate surgery→RT intervals** as core quality indicators.

5. **Patients starting RT 16–17 weeks post-surgery are not outliers** in Alberta’s current system—they represent structural constraints, not stochastic variation.

6. **Persistent delays risk worsening population-level cancer outcomes** via increased local recurrence, particularly among higher-risk tumor subtypes.

---

# 9. Recent and Planned Expansions at the Cross Cancer Institute

In recent years, the Cross Cancer Institute (CCI) has initiated several major expansions and infrastructure upgrades — efforts that may improve future throughput and reduce pathway bottlenecks.

- As of **November 3, 2025**, some outpatient and consult appointments are being routed through a newly established **CCI Outpatient Satellite Clinic**.
- During 2024–2025, CCI completed renovations that include a new high-dose radiotherapy vault, a modernized brachytherapy suite, a dedicated staff wellness area, and a virtual-care centre, and has announced plans for expanded systemic therapy capacity, a larger clinical trials unit, an expanded outpatient clinic, and an upgraded pharmacy.
- Additionally, one of the older linear-accelerator vaults (Vault 8) was recently retrofitted with a modern multi-energy linac (with contemporary imaging capabilities), improving treatment capacity and aligning with current standard-of-care technology.

These developments reflect institutional recognition of the capacity and access limitations described earlier. While the impact on surgery-to-RT wait times remains to be seen, these capital investments and site expansions represent important system-level interventions that could — if matched by appropriate operational staffing and scheduling reform — help reduce consult and treatment delays in the medium term.

> **Note:** Since the primary bottleneck identified in Alberta remains the referral → consult interval, expansions of physical space (outpatient clinics, vaults, satellite sites) are a necessary first step — but must be paired with increases in clinician and support-staff manpower to meaningfully shift surgery→RT timelines.

# Conclusion

Timely initiation of adjuvant radiotherapy after breast-conserving surgery is a core quality indicator in breast cancer care, supported by decades of evidence demonstrating that delays beyond 12 weeks compromise local control and, when prolonged to 20 weeks or more, may adversely affect survival. The Canadian cancer system is designed around this principle; yet, real-world performance varies across provinces due to differing structural constraints, workforce capacity, and operational design.

Alberta’s current challenge is not treatment throughput but access to consultation. The prolonged interval from referral to radiation oncology assessment—routinely exceeding 12 weeks in Edmonton—creates a clinically significant extension of surgery→RT timelines despite strong performance in the Ready→Start segment. By contrast, British Columbia’s 2022–2023 crisis originated in system-wide treatment capacity limitations, which were subsequently reversed through intensive investment in staffing, physical infrastructure, and strategic provincial prioritization.

Recent developments at the Cross Cancer Institute—including a newly opened Outpatient Satellite Clinic, renovation and modernization of RT vaults and brachytherapy suites, expansion of outpatient and virtual-care space, and plans for increased clinical trial and systemic therapy capacity—demonstrate institutional recognition of the need to expand infrastructure to meet rising demand. These investments are critical foundational steps. However, infrastructure alone will not shorten surgery→RT intervals unless accompanied by proportionate increases in physician FTEs, clinic scheduling capacity, and allied health staffing.

For radiation oncology leadership, the key insight is that operational metrics must not substitute for clinically meaningful indicators. A system may meet official 28-day treatment benchmarks while simultaneously delivering care outside evidence-based therapeutic windows. Therefore, routine monitoring of surgery→RT intervals, stratified by chemotherapy status and tumor biology, should be incorporated into provincial quality-improvement frameworks. Such measurement would align performance monitoring with clinical practice realities and enable targeted interventions at the pathway segments where delays are actually occurring.

Ultimately, improving timeliness of adjuvant radiotherapy in Alberta—and maintaining recovered performance in British Columbia—will require coordinated investment across the entire cancer pathway: from surgical scheduling and referral workflows to consult capacity, planning infrastructure, and treatment delivery. Sustaining high-quality breast cancer care will depend on the extent to which provincial cancer programs can align physical expansion with workforce planning, ensuring that every patient requiring post-lumpectomy radiotherapy receives it within the 6–12 week evidence-based window that optimizes oncologic outcomes.

# References

# References

1. Steering Committee on Clinical Practice Guidelines for the Care and Treatment of Breast Cancer. *Clinical practice guidelines for the care and treatment of breast cancer.* Health Canada; 1998.

2. Cancer Care Ontario. *Breast Cancer Management Guidelines.* Toronto, ON; various guideline updates.

3. National Comprehensive Cancer Network (NCCN). *Clinical Practice Guidelines in Oncology: Breast Cancer.* Versions 2023–2024.

4. European Society for Radiotherapy & Oncology (ESTRO). *Radiotherapy of Breast Cancer—Professional Guideline.* 2017.

5. Netherlands Comprehensive Cancer Organisation (IKNL). *National Breast Cancer Guideline (Richtlijn Mammacarcinoom).* 2012–2017.

6. Huang J, Barbera L, Brouwers M, Browman GP, Mackillop WJ. Does delay in starting treatment affect outcomes of radiotherapy? A systematic review. *J Clin Oncol.* 2003;21(3):555–563.

7. Chen Z, King W, Pearcey R, Kerba M, Mackillop WJ. The relationship between radiotherapy waiting time and clinical outcomes: A systematic review. *Radiother Oncol.* 2008;87(1):3–16.

8. Olivotto IA, Lesperance ML, Truong PT, et al. Intervals longer than 20 weeks from breast-conserving surgery to radiation therapy are associated with inferior outcomes. *J Clin Oncol.* 2009;27(1):16–23.

9. van Maaren MC, Strobbe LJA, Poortmans PMP, et al. Timing of radiotherapy following breast-conserving surgery and its impact on disease-free survival. *Br J Cancer.* 2017;116:52–60.

10. Canadian Partnership Against Cancer (CPAC). *Wait Times for Cancer Treatment in Canada: Pan-Canadian Benchmarks.* Toronto; 2007–2024.

11. Canadian Institute for Health Information (CIHI). *Wait Times for Radiation Therapy: Technical Notes & Annual Updates.* Ottawa; 2019–2024.

12. Alberta Health Services, Cancer Care Alberta. *Cancer Treatment Wait Time Reporting.* Edmonton; multiple annual reports.

13. Alberta Medical Association. *Oncology Access and Wait Time Pressures in Alberta.* AMA Briefing; 2023–2024.

14. Alberta Health Services. *Future of Cancer Impact in Alberta – Full Report.* Edmonton; 2022.

15. BC Cancer. *Radiation Therapy Wait Time Performance Dashboard.* Vancouver; 2019–2025.

16. Government of British Columbia (BC Gov News). *Province launches 10-year Cancer Plan; radiation therapy performance updates.* Press releases 2023–2025.

17. British Columbia Medical Journal. Deteriorating wait times for breast cancer patients at a regional hospital in BC (2013 vs 2023). *BCMJ.* 2023.

18. Chiarelli AM, Shi L, Halapy E, et al. Factors associated with wait times across the breast cancer treatment pathway in Ontario. *SpringerPlus.* 2013;2:388.

19. Canadian Cancer Society. *Radiation Therapy for Breast Cancer: Clinical Overview.* Accessed 2023–2025.

20. Alberta Health Services. *Cross Cancer Institute — Locations and Outpatient Satellite Clinic.* Updated Nov 3, 2025.  
    https://www.albertahealthservices.ca/cci/cci.aspx

21. Alberta Cancer Foundation. *A Transformed Space Means Improved Cancer Care and Research (Cross Cancer Institute Renovation & Expansion Update).* 2024.  
    https://albertacancer.ca/rtc2024/a-transformed-space-means-improved-cancer-care-and-research/

22. ACI Architecture. *Cross Cancer Institute – Vault 8 Linear Accelerator Retrofit (Technical Project Summary).* 2024.  
    https://www.aci-arch.com/projects/cross-cancer-institute-vault-8-linear-accelerator/
